...
`OPENAI_API_KEY`가 있으면 첫 Responses API 호출이 `correct_answer`, `false_answer`, `false_basis`, `level_fit_reason`, `student_answer`를 생성합니다. 두 번째 독립 verifier 호출은 교사 승인 baseline을 기준으로 정답 일치, 실제 거짓 여부, 학생 답변 내 거짓 주장 포함, Level 적합, 진실 맥락 혼합, 정답·정정 누출, 중학생 대상 미묘함을 검사합니다. 모든 verifier 조건이 통과해야 학생에게 전송하며 실패 시 최대 3회 재생성합니다.
