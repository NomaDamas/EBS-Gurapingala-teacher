...
const checks = [
  ...
  ["health endpoint is safe and ready", async () => {
    ...
    return res.status === 200 &&
      ...
      body.defaultFalseDensity === "single" &&
  ...
  ["debrief csv export is room aware and complete", async () => {
    ...
    return res.status === 200 &&
      ...
      [
        ...
        "PASS_HARD_GATES_WITH_QUALITY_WARNING",
        "PASS_STRICT_DB_LLM_VERIFIED"
      ].includes(verifyRow?.preflightVerdict) &&
...
async function getHealthEvidence() {
  try {
    ...
    return {
      ...
      defaultFalseDensity: safeString(body.defaultFalseDensity),
