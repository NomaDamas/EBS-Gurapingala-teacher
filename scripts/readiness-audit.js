...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["src/worker.js"], ["export class ClassroomRoom", "/api/health", "buildHealthPayload", "openaiModel", "defaultRoomId", "SECURITY_HEADERS", "cache-control", "no-store", "x-content-type-options", "referrer-policy"]) &&
      includesAll(files["docs/deployment-guide.md"], ["npm run deploy", "Deploy", "CLOUDFLARE_API_TOKEN", "npm run verify:deploy", "Node.js 22", "npm ci", "EXPECTED_OPENAI_MODEL", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "production `Deploy` workflow에서는 필수", "x-teacher-token", "촬영방 분리", "VERIFY_ROOM=deploy-verify", "실제 촬영방 금지", "cache-control: no-store"]) &&
      ...
      includesAll(files["scripts/verify-deploy.js"], ["deploy verification passed", "/api/evaluation-set", "/api/evaluation-set/full", "/api/join", "/api/chat", "/api/export", "/api/purge", "/teacher", "teacher page access policy is enforced", "teacher token is configured when required", "OpenAI model matches expectation when provided", "EXPECTED_OPENAI_MODEL", "WORKER_ROOM", "VERIFY_ROOM", "deploy-verify", "ALLOW_PURGE_FILMING_ROOM", "REQUIRE_OPENAI", "REQUIRE_TEACHER_TOKEN", "x-teacher-token", "x-purge-room"]) &&
  ...
  },
  {
    ...
    run: async (files) =>
      includesAll(files["docs/production-runbook.md"], ["촬영 전날", "리허설", "촬영 중", "촬영 직후", "데이터 삭제", "Node.js 22", "npm ci", "openaiModel", "EXPECTED_OPENAI_MODEL", "/api/health", "/api/debrief.csv"]) &&
