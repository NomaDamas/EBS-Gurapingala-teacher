...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["docs/external-review-packet.md"], ["GPT-5.5 xhigh", "중학생은 멍청이가 아니므로", "학생 화면에는 Level에 맞춘 거짓 답변만", "교사 화면과 export에는 정답, 거짓, 왜 거짓인지", "반드시 반려할 조건", "correctAnswer", "VERIFY_ROOM", "EXPECTED_OPENAI_TIMEOUT_MS", "unsafe_persona_instruction", "teacher_config_rejected", "x-robots-tag: noindex, nofollow", "Review decision: APPROVE | REQUEST_CHANGES", "rehearsal:config against each filming room", "classroom-config-evidence/v1", "CLASSROOM_CONFIG_STATUS=pass", "external-review-evidence/v1", "npm run review:evidence", "npm run review:packet", "blocking finding", "npm run preflight:deploy", "npm run verify:deploy", "npm run release:audit"]) &&
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷", "unsafe_persona_instruction", "x-robots-tag: noindex, nofollow", "npm run review:evidence", "npm run review:packet", "CLASSROOM_CONFIG_STATUS=pass", "external-review-evidence/v1", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "증거 파일의 `roomId` 집합", "npm run release:audit"]) &&
      includesAll(files["docs/deployment-guide.md"], ["npm run review:evidence", "npm run review:packet", "npm run release:audit", "EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "CLASSROOM_CONFIG_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "external-review-evidence/v1", "evidenceChecked", "blockingFindings", "VERIFY_DEPLOY_EVIDENCE_FILE", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "증거 파일의 `roomId` 집합", "deploy-verification-evidence/v1", "classroom-config-evidence/v1"]) &&
      ...
      includesAll(files["scripts/print-external-review-packet.js"], ["PR_URL", "Latest PR head SHA", "GPT-5.5 xhigh", "학생 화면에는 Level에 맞춘 거짓 답변만", "release:commands", "TEACHER_TOKEN 원문", "Review decision: APPROVE | REQUEST_CHANGES", "EXTERNAL_REVIEW_DECISION=APPROVE", "npm run review:evidence"]) &&
