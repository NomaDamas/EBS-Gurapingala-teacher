...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["scripts/verify-classroom-config.js"], ["classroom-config-evidence/v1", "CLASSROOM_ROOM", "EXPECTED_FALSE_LEVEL", "EXPECTED_PERSONA", "APPLY_CLASSROOM_CONFIG", "CLASSROOM_CONFIG_EVIDENCE_FILE", "PR_HEAD_SHA or GITHUB_SHA is required", "EXPECTED_OPENAI_MODEL is required when REQUIRE_OPENAI=true", "EXPECTED_OPENAI_TIMEOUT_MS", "expectedOpenAITimeoutMs", "openaiTimeoutMs", "sharingUrls", "studentUrlHasToken", "teacherUrlRequiresToken", "observedHealth", "CLASSROOM_ROOM must be a filming/rehearsal room, not deploy-verify", "classroom Level/persona matches expected config", "x-teacher-token", "cache-control", "x-robots-tag"]) &&
      ...
      includesAll(files["scripts/verify-github-deploy-setup.js"], ["CLOUDFLARE_ACCOUNT_ID", "CLOUDFLARE_API_TOKEN", "OPENAI_API_KEY", "TEACHER_TOKEN", "WORKER_HEALTH_URL", "EXPECTED_OPENAI_MODEL", "EXPECTED_OPENAI_TIMEOUT_MS", "gh secret set", "gh variable set", "secretValuesPrinted=false", "github deploy setup verification failed"]) &&
      includesAll(files["scripts/print-release-commands.js"], ["CLASSROOM_PLANS", "PR_HEAD_SHA or GITHUB_SHA is required", "npm run verify:ci", "npm run verify:deploy", "npm run rehearsal:config", "npm run review:evidence", "npm run release:audit", "EXPECTED_OPENAI_TIMEOUT_MS", "CI_EVIDENCE_FILE=artifacts/ci-evidence.json", "VERIFY_DEPLOY_EVIDENCE_FILE=artifacts/deploy-evidence.json", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "deploy-verify", "roomId must be a filming room"]) &&
