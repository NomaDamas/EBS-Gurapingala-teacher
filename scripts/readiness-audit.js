...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["scripts/print-release-commands.js"], ["CLASSROOM_PLANS", "PR_HEAD_SHA or GITHUB_SHA is required", "npm run verify:deploy", "npm run rehearsal:config", "npm run review:evidence", "npm run release:audit", "VERIFY_DEPLOY_EVIDENCE_FILE=artifacts/deploy-evidence.json", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "deploy-verify", "roomId must be a filming room"]) &&
