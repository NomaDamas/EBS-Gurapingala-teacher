...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      includesAll(files["docs/launch-audit.md"], ["학생은 로그인 없이 URL과 이름만으로 입장", "교사는 학생 카드별 online/offline과 채팅 진행 상태를 실시간 관찰", "교사 Level/persona 설정이 실제 학생 답변 생성과 export 타임라인에 반영", "persona가 정답 공개", "unsafe_persona_instruction", "teacher_config_rejected", "production 배포는 교사용 token 보호를 강제", "CI와 Deploy가 같은 고정 의존성 그래프를 사용", "production 배포는 실제 Worker URL 검증을 생략하지 않음", "배포 전 환경 누락을 사전에 차단", "npm run preflight:deploy", "촬영 기대 모델이 아닌 다른 OpenAI 모델로 배포", "OpenAI 요청 지연", "openaiTimeoutMs", "Cloudflare 계정이 여러 개", "CLOUDFLARE_ACCOUNT_ID", "민감 응답이 브라우저/프록시에 캐시되지 않고", "x-robots-tag: noindex, nofollow", "학생 화면에는 Level에 맞춘 거짓 답변만 표시", "preflight 실패 audit가 학생 응답으로 누출되지 않음", "공개 평가 endpoint가 정답·거짓 근거를 누출하지 않음", "진실과 거짓이 섞이고 너무 쉬운 거짓으로만 흐르지 않음", "배포 검증이 실제 촬영방 로그를 삭제하지 않음", "GitHub Deploy workflow", "GPT-5.5 xhigh 또는 동등한 외부 코드 리뷰 승인", "external-review-packet.md", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "REQUIRE_CLASSROOM_CONFIG=true", "CLASSROOM_CONFIG_EVIDENCE_FILE", "VERIFY_ROOM=deploy-verify", "x-purge-room", "npm run release:audit", "EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "PR_HEAD_SHA"]) &&
      includesAll(files["docs/external-review-packet.md"], ["GPT-5.5 xhigh", "중학생은 멍청이가 아니므로", "학생 화면에는 Level에 맞춘 거짓 답변만", "교사 화면과 export에는 정답, 거짓, 왜 거짓인지", "반드시 반려할 조건", "correctAnswer", "VERIFY_ROOM", "EXPECTED_OPENAI_TIMEOUT_MS", "unsafe_persona_instruction", "teacher_config_rejected", "x-robots-tag: noindex, nofollow", "Review decision: APPROVE | REQUEST_CHANGES", "external-review-evidence/v1", "npm run review:evidence", "blocking finding", "npm run preflight:deploy", "npm run verify:deploy", "npm run release:audit"]) &&
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷", "unsafe_persona_instruction", "x-robots-tag: noindex, nofollow", "npm run review:evidence", "CLASSROOM_CONFIG_EVIDENCE_FILE", "npm run release:audit"]) &&
      includesAll(files["docs/deployment-guide.md"], ["npm run review:evidence", "npm run release:audit", "EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE", "CLASSROOM_CONFIG_EVIDENCE_FILE", "deploy-verification-evidence/v1", "classroom-config-evidence/v1"]) &&
      includesAll(files["scripts/release-audit.js"], ["EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE", "CLASSROOM_CONFIG_EVIDENCE_FILE", "deploy-verification-evidence/v1", "classroom-config-evidence/v1", "CLASSROOM_CONFIG_EVIDENCE_FILE prHeadSha must match PR_HEAD_SHA", "CLASSROOM_CONFIG_EVIDENCE_FILE roomId must be a filming/rehearsal room", "CLASSROOM_CONFIG_EVIDENCE_FILE observedConfig must match expected Level/persona", "REQUIRE_CLASSROOM_CONFIG=true", "EXTERNAL_REVIEW_FILE prHeadSha must match PR_HEAD_SHA", "VERIFY_DEPLOY_EVIDENCE_FILE prHeadSha must match PR_HEAD_SHA", "VERIFY_DEPLOY_EVIDENCE_FILE must record requireOpenAI=true", "VERIFY_DEPLOY_EVIDENCE_FILE must record requireTeacherToken=true", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "release audit failed"]) &&
      includesAll(files["scripts/write-external-review-evidence.js"], ["external-review-evidence/v1", "EXTERNAL_REVIEW_DECISION", "EXTERNAL_REVIEWER", "PR_HEAD_SHA", "CI_STATUS", "TESTS_STATUS", "EVAL_STATUS", "READINESS_STATUS", "SMOKE_STATUS", "APPROVE evidence cannot include BLOCKING_FINDINGS", "external review evidence written"]) &&
      includesAll(files["test/external-review-evidence.test.js"], ["review:evidence writes structured approval evidence", "review:evidence rejects approval with blocking findings", "review:evidence fails closed"]) &&
