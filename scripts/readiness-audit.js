...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷", "unsafe_persona_instruction", "x-robots-tag: noindex, nofollow", "npm run review:evidence", "CLASSROOM_CONFIG_EVIDENCE_FILES", "npm run release:audit"]) &&
      includesAll(files["docs/deployment-guide.md"], ["npm run review:evidence", "npm run release:audit", "EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE", "CLASSROOM_CONFIG_EVIDENCE_FILES", "deploy-verification-evidence/v1", "classroom-config-evidence/v1"]) &&
      includesAll(files["scripts/release-audit.js"], ["EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE", "CLASSROOM_CONFIG_EVIDENCE_FILES", "deploy-verification-evidence/v1", "classroom-config-evidence/v1", "CLASSROOM_CONFIG_EVIDENCE_FILE ${file}", "prHeadSha must match PR_HEAD_SHA", "roomId must be a filming/rehearsal room", "observedConfig must match expected Level/persona", "roomId must be unique across CLASSROOM_CONFIG_EVIDENCE_FILES", "REQUIRE_CLASSROOM_CONFIG=true", "EXTERNAL_REVIEW_FILE prHeadSha must match PR_HEAD_SHA", "VERIFY_DEPLOY_EVIDENCE_FILE prHeadSha must match PR_HEAD_SHA", "VERIFY_DEPLOY_EVIDENCE_FILE must record requireOpenAI=true", "VERIFY_DEPLOY_EVIDENCE_FILE must record requireTeacherToken=true", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "release audit failed"]) &&
