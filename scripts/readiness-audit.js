...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["wrangler.toml"], ["durable_objects.bindings", "ClassroomRoom", "CHAT_RATE_LIMIT_PER_MINUTE", 'DEFAULT_FALSE_DENSITY = "single"', 'EVENT_TTL_HOURS = "0"']) &&
      includesAll(files["src/worker.js"], ["export class ClassroomRoom", "/api/health", "/api/config", "writeConfig", "teacher_config_updated", "recordEvent(event)", "persistentEventStorageKey", 'storage.list({ prefix: "event:" })', 'eventRetentionMode: "manual"', "eventTtlHours: 0", "buildHealthPayload", "openaiModel", "openaiVerifierModel", "openaiTimeoutMs", "defaultRoomId", "SECURITY_HEADERS", "function text", "text/plain; charset=utf-8"]) &&
