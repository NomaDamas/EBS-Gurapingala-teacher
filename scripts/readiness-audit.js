...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["src/worker.js"], ["export class ClassroomRoom", "/api/health", "/api/config", "writeConfig", "teacher_config_updated", "recordEvent(event)", "buildHealthPayload", "openaiModel", "defaultRoomId", "SECURITY_HEADERS", "cache-control", "no-store", "x-content-type-options", "referrer-policy"]) &&
      ...
      includesAll(files["scripts/smoke-worker.js"], ["worker smoke passed", "/api/chat", "/api/export", "/api/health", "/api/config", "teacher config API controls generated audit level", "teacher_config_updated", "room query isolates classroom events", "x-content-type-options", "referrer-policy"]) &&
      includesAll(files["scripts/verify-deploy.js"], ["deploy verification passed", "/api/evaluation-set", "/api/evaluation-set/full", "/api/config", "/api/join", "/api/chat", "/api/export", "/api/purge", "/teacher", "teacher page access policy is enforced", "teacher token is configured when required", "teacher config API controls generated audit level", "teacher_config_updated", "OpenAI model matches expectation when provided", "EXPECTED_OPENAI_MODEL", "WORKER_ROOM", "VERIFY_ROOM", "deploy-verify", "ALLOW_PURGE_FILMING_ROOM", "REQUIRE_OPENAI", "REQUIRE_TEACHER_TOKEN", "x-teacher-token", "x-purge-room"]) &&
