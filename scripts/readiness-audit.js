...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷", "unsafe_persona_instruction", "x-robots-tag: noindex, nofollow", "npm run review:evidence", "npm run review:packet", "CLASSROOM_CONFIG_STATUS=pass", "external-review-evidence/v1", "Cloudflare response header evidence", "sanitized `/api/health` snapshot", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "증거 파일의 `roomId` 집합", "EXPECTED_OPENAI_MODEL=gpt-5.5 EXPECTED_OPENAI_TIMEOUT_MS=15000 PR_HEAD_SHA=<latest-sha>", "npm run release:audit"]) &&
