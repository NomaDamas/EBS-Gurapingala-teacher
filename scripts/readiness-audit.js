...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷", "unsafe_persona_instruction", "x-robots-tag: noindex, nofollow", "npm run release:audit"]) &&
      includesAll(files["docs/deployment-guide.md"], ["npm run release:audit", "EXTERNAL_REVIEW_DECISION=APPROVE", "VERIFY_DEPLOY_STATUS=pass", "EXPECTED_PR_HEAD_SHA", "EXTERNAL_REVIEW_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE", "deploy-verification-evidence/v1"]) &&
