...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      includesAll(files["scripts/run-eval.js"], ["model-evaluation-evidence/v1", "REQUIRE_OPENAI_EVAL", "LLM_PROVIDER=openai", "all 50 turns must be approved by the independent OpenAI verifier", "zero provider or judge fallback turns", "unique across all 150 calls", "evaluationSetSha256", "responseId", "responseModel"]) &&
      ...
      includesAll(files[".github/workflows/deploy.yml"], ["id-token: write", "attestations: write", "REQUIRE_OPENAI_EVAL: \"true\"", "LLM_PROVIDER: openai", "model-evaluation-evidence", "EVAL_OUTPUT: artifacts/model-evaluation-evidence.json", "OPENAI_VERIFIER_MODEL", "EVAL_JUDGE_MODEL", "actions/attest@v4", "subject-path: artifacts/model-evaluation-evidence.json", "gpt-5.6-terra"]) &&
