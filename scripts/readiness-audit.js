...
const checks = [
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["src/worker.js"], ['url.pathname === "/"', 'url.pathname === "/api/join"', "normalizeRoomId", "MAX_JSON_BODY_BYTES", "payload_too_large", "registerStudentSession", "validateStudentSession", "session_conflict", "session_verification_failed"])
  },
  {
    id: "teacher-realtime-dashboard",
    evidence: ["src/ui/teacher.js", "src/worker.js"],
    run: async (files) =>
      includesAll(files["src/ui/teacher.js"], ["/ws/teacher", "학생 카드", "teacher_config", "snapshot", "실시간 연결 재시도", "reconnectAttempts", "roomId", "withRoom", "applyTeacherConfig", "postTeacherConfig", "/api/config", "저장 실패: 네트워크 확인", "teacher_config_updated", "학생 URL 복사", "교사용 URL 복사", "history.replaceState", "latencyMs", "blockedForStudent", "blockedMsg", "document.createTextNode(session.name)", "설정 적용 상태", "configStatus", "classSummary", "summaryMetric", "replaceChildren", "채팅턴"]) &&
      !files["src/ui/teacher.js"].includes("classSummaryEl.innerHTML") &&
  ...
  },
  {
    ...
    evidence: ["wrangler.toml", "src/worker.js", "src/domain/security.js", "docs/deployment-guide.md", "scripts/smoke-worker.js", "scripts/verify-deploy.js", "scripts/deploy-preflight.js", "test/deploy-preflight.test.js", ".github/workflows/ci.yml", ".github/workflows/deploy.yml", "package.json", "package-lock.json"],
    run: async (files) =>
      ...
      includesAll(files["docs/deployment-guide.md"], ["npm run deploy", "Deploy", "CLOUDFLARE_API_TOKEN", "CLOUDFLARE_ACCOUNT_ID", "npm run verify:deploy", "Node.js 22", "npm ci", "EXPECTED_OPENAI_MODEL", "EXPECTED_OPENAI_TIMEOUT_MS", "OPENAI_TIMEOUT_MS", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "Level/persona 설정 API", "production `Deploy` workflow에서는 필수", "x-teacher-token", "Sec-WebSocket-Protocol", "WebSocket URL에는 token이 남지 않아야", "teacher websocket accepts subprotocol token without query token", "촬영방 분리", "VERIFY_ROOM=deploy-verify", "실제 촬영방 금지", "cache-control: no-store", "content-security-policy", "permissions-policy"]) &&
      includesAll(files["scripts/smoke-worker.js"], ["worker smoke passed", "/api/chat", "/api/export", "/api/health", "/api/config", "error responses include security headers", "oversized student JSON returns 413 before validation", "teacher config API controls generated audit level", "teacher_config_updated", "room query isolates classroom events", "x-content-type-options", "referrer-policy", "content-security-policy", "permissions-policy"]) &&
      includesAll(files["scripts/verify-deploy.js"], ["deploy verification passed", "/api/evaluation-set", "/api/evaluation-set/full", "/api/config", "/api/join", "/api/chat", "/api/export", "/api/purge", "/teacher", "/ws/teacher", "교사용 대시보드에 기록됩니다", "이름 외 개인정보는 입력하지 마세요", "teacher page access policy is enforced", "teacher token is configured when required", "teacher config API controls generated audit level", "teacher websocket accepts subprotocol token without query token", "teacher_config_updated", "OpenAI model matches expectation when provided", "OpenAI timeout matches expectation when provided", "EXPECTED_OPENAI_MODEL", "EXPECTED_OPENAI_TIMEOUT_MS", "openaiTimeoutMs", "WORKER_ROOM", "VERIFY_ROOM", "deploy-verify", "ALLOW_PURGE_FILMING_ROOM", "REQUIRE_OPENAI", "REQUIRE_TEACHER_TOKEN", "x-teacher-token", "sec-websocket-protocol", "x-purge-room"]) &&
  ...
  },
  {
    ...
    run: async (files) =>
      includesAll(files["docs/production-runbook.md"], ["촬영 전날", "리허설", "촬영 중", "촬영 직후", "데이터 삭제", "Node.js 22", "npm ci", "CLOUDFLARE_ACCOUNT_ID", "openaiModel", "openaiTimeoutMs", "EXPECTED_OPENAI_MODEL", "EXPECTED_OPENAI_TIMEOUT_MS", "/api/config", "teacher_config_updated", "/api/health", "/api/debrief.csv"]) &&
  ...
  },
  {
    ...
    evidence: ["docs/launch-audit.md", "docs/external-review-packet.md", "README.md", "docs/implementation-plan.md"],
    run: async (files) =>
      includesAll(files["docs/launch-audit.md"], ["학생은 로그인 없이 URL과 이름만으로 입장", "교사는 학생 카드별 online/offline과 채팅 진행 상태를 실시간 관찰", "교사 Level/persona 설정이 실제 학생 답변 생성과 export 타임라인에 반영", "production 배포는 교사용 token 보호를 강제", "CI와 Deploy가 같은 고정 의존성 그래프를 사용", "production 배포는 실제 Worker URL 검증을 생략하지 않음", "배포 전 환경 누락을 사전에 차단", "npm run preflight:deploy", "촬영 기대 모델이 아닌 다른 OpenAI 모델로 배포", "OpenAI 요청 지연", "openaiTimeoutMs", "Cloudflare 계정이 여러 개", "CLOUDFLARE_ACCOUNT_ID", "민감 응답이 브라우저/프록시에 캐시되지 않음", "학생 화면에는 Level에 맞춘 거짓 답변만 표시", "preflight 실패 audit가 학생 응답으로 누출되지 않음", "공개 평가 endpoint가 정답·거짓 근거를 누출하지 않음", "진실과 거짓이 섞이고 너무 쉬운 거짓으로만 흐르지 않음", "배포 검증이 실제 촬영방 로그를 삭제하지 않음", "GitHub Deploy workflow", "GPT-5.5 xhigh 또는 동등한 외부 코드 리뷰 승인", "external-review-packet.md", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "VERIFY_ROOM=deploy-verify", "x-purge-room"]) &&
      includesAll(files["docs/external-review-packet.md"], ["GPT-5.5 xhigh", "중학생은 멍청이가 아니므로", "학생 화면에는 Level에 맞춘 거짓 답변만", "교사 화면과 export에는 정답, 거짓, 왜 거짓인지", "반드시 반려할 조건", "correctAnswer", "VERIFY_ROOM", "EXPECTED_OPENAI_TIMEOUT_MS", "Review decision: APPROVE | REQUEST_CHANGES", "npm run preflight:deploy", "npm run verify:deploy"]) &&
      includesAll(files["README.md"], ["프로덕션 런치 감사 매트릭스", "GPT-5.5 xhigh 외부 리뷰 패킷"]) &&
