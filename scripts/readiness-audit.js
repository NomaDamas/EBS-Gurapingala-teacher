...
const checks = [
  ...
  },
  {
    ...
    evidence: ["wrangler.toml", "src/worker.js", "src/domain/security.js", "docs/deployment-guide.md", "scripts/smoke-worker.js", "scripts/verify-deploy.js", "scripts/verify-classroom-config.js", "scripts/deploy-preflight.js", "scripts/print-release-commands.js", "scripts/print-classroom-urls.js", "scripts/print-shoot-checklist.js", "test/deploy-preflight.test.js", "test/deploy-workflow.test.js", "test/classroom-config.test.js", "test/release-commands.test.js", "test/classroom-urls.test.js", "test/shoot-checklist.test.js", ".github/workflows/ci.yml", ".github/workflows/deploy.yml", "package.json", "package-lock.json"],
    run: async (files) =>
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["scripts/write-external-review-evidence.js"], ["external-review-evidence/v1", "EXTERNAL_REVIEW_DECISION", "EXTERNAL_REVIEWER", "EXTERNAL_REVIEW_SOURCE_URL", "EXTERNAL_REVIEW_TRANSCRIPT_FILE", "transcriptSha256", "PR_HEAD_SHA", "CI_STATUS", "TESTS_STATUS", "EVAL_STATUS", "READINESS_STATUS", "SMOKE_STATUS", "VERIFY_DEPLOY_STATUS", "CLASSROOM_CONFIG_STATUS", "EXPECTED_CLASSROOM_ROOMS is required", "validateExpectedClassroomRooms", "missing expected filming room", "contains unexpected filming room", "contains duplicate filming room", "VERIFY_DEPLOY_EVIDENCE_FILE is required", "CLASSROOM_CONFIG_EVIDENCE_FILES or CLASSROOM_CONFIG_EVIDENCE_FILE is required", "evidenceArtifacts", "sha256", "APPROVE evidence cannot include BLOCKING_FINDINGS", "validateApprovalEvidenceArtifacts", "validateCiEvidenceArtifact", "CI_EVIDENCE_FILE checkRun.completedAt must be after checkRun.startedAt", "CI_EVIDENCE_FILE generatedAt must be after checkRun.completedAt", "status must be pass", "prHeadSha must match PR_HEAD_SHA", "roomId must be a filming/rehearsal room", "external review evidence written"]) &&
      includesAll(files["scripts/print-external-review-packet.js"], ["PR_URL", "Latest PR head SHA", "GPT-5.5 xhigh", "학생 화면에는 Level에 맞춘 거짓 답변만", "release:commands", "TEACHER_TOKEN 원문", "Review decision: APPROVE | REQUEST_CHANGES", "EXTERNAL_REVIEW_DECISION=APPROVE", "EXTERNAL_REVIEW_TRANSCRIPT_FILE", "VERIFY_DEPLOY_EVIDENCE_FILE=artifacts/deploy-evidence.json", "CLASSROOM_CONFIG_EVIDENCE_FILES=artifacts/2026-07-13-3-5-config.json,artifacts/2026-07-16-3-1-config.json", "EXPECTED_CLASSROOM_ROOMS=2026-07-13-3-5,2026-07-16-3-1", "npm run review:evidence"]) &&
      includesAll(files["test/external-review-evidence.test.js"], ["review:evidence writes structured approval evidence", "CLASSROOM_CONFIG_STATUS", "classroomConfigStatus", "review:evidence rejects approval with blocking findings", "review:evidence rejects approval when CI evidence timestamps are invalid", "review:evidence rejects approval when expected classroom room evidence is missing", "review:evidence rejects approval when classroom room evidence is unexpected or duplicate", "review:evidence fails closed"]) &&
      ...
      includesAll(files["test/external-review-packet.test.js"], ["review:packet prints current PR target", "PR_HEAD_SHA=abc123", "EXPECTED_CLASSROOM_ROOMS=2026-07-13-3-5,2026-07-16-3-1", "npm run review:evidence"]) &&
      includesAll(files["package.json"], ["verify:ci", "review:evidence", "review:packet", "release:audit"]) &&
      includesAll(files[".github/pull_request_template.md"], ["Main Goal", "Required Evidence", "Release Gates", "Safety Review", "GPT-5.5 xhigh", "npm run verify:deploy", "npm run rehearsal:config", "classroom-config-evidence/v1", "REQUIRE_OPENAI=true", "REQUIRE_TEACHER_TOKEN=true", "REQUIRE_CLASSROOM_CONFIG=true", "REQUIRE_CLOUDFLARE_EDGE=true", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "evidence `roomId` set", "CLASSROOM_ROOM", "CI_HEAD_SHA", "npm run release:audit", "Do not merge", "correctAnswer", "whyFalse", "VERIFY_ROOM", "Debrief JSON/CSV"]) &&
