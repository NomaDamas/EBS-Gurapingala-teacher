...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["scripts/print-release-commands.js"], ["CLASSROOM_PLANS", "npm run verify:deploy", "npm run rehearsal:config", "npm run review:evidence", "npm run release:audit", "VERIFY_DEPLOY_EVIDENCE_FILE=artifacts/deploy-evidence.json", "CLASSROOM_CONFIG_EVIDENCE_FILES", "EXPECTED_CLASSROOM_ROOMS", "deploy-verify", "roomId must be a filming room"]) &&
      ...
      includesAll(files["package.json"], ["\"node\": \">=22.0.0\"", "preflight:deploy", "verify:deploy", "rehearsal:config", "release:commands"]) &&
