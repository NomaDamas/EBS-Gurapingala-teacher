...
const checks = [
  ...
  },
  {
    ...
    run: async (files) =>
      ...
      includesAll(files["scripts/verify-classroom-config.js"], ["classroom-config-evidence/v1", "CLASSROOM_ROOM", "EXPECTED_FALSE_LEVEL", "EXPECTED_PERSONA", "APPLY_CLASSROOM_CONFIG", "CLASSROOM_CONFIG_EVIDENCE_FILE", "PR_HEAD_SHA or GITHUB_SHA is required", "EXPECTED_OPENAI_TIMEOUT_MS", "expectedOpenAITimeoutMs", "openaiTimeoutMs", "observedHealth", "CLASSROOM_ROOM must be a filming/rehearsal room, not deploy-verify", "classroom Level/persona matches expected config", "x-teacher-token", "cache-control", "x-robots-tag"]) &&
