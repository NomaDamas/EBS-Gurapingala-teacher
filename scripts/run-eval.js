...
const evaluationItems = EVALUATION_SET_50.slice(0, evaluationTurnCount).map((item) => fixedLevel
...
const payload = {
  ...
  evaluationSetSha256: createHash("sha256")
    ...
    .digest("hex"),
  selectionCriteria: {
    primary: `All ${evaluationItems.length} turns must pass with zero truth leak, then maximize falsehoodRate, levelFitRate, averageSubtletyScore, and teacherAuditQuality.`,
    minimums: {
      falsehoodRate: 1,
      levelFitRate: 1,
      truthLeakRate: 0,
...
function normalizeEvaluationTurnCount(value) {
  if (value === undefined || value === null || value === "") return EVALUATION_SET_50.length;
  const count = Number(value);
  if (!Number.isInteger(count) || count < 1 || count > EVALUATION_SET_50.length) {
    throw new Error(`EVAL_TURN_COUNT must be an integer between 1 and ${EVALUATION_SET_50.length}.`);
  }
  return count;
}
...
function validateProductionResults(items, selected) {
  ...
  if (expected.total !== evaluationItems.length ||
    expected.passed !== evaluationItems.length ||
    expected.failed !== 0) {
    failures.push(`EXPECTED_OPENAI_MODEL must pass all ${evaluationItems.length} evaluation turns`);
  }
  if (expected.execution?.openaiGeneratedTurns !== evaluationItems.length) {
    failures.push(`all ${evaluationItems.length} turns must be generated by OpenAI`);
  }
  if (expected.execution?.openaiVerifiedTurns !== evaluationItems.length) {
    failures.push(`all ${evaluationItems.length} turns must be approved by the independent OpenAI verifier`);
  }
  if (expected.execution?.openaiJudgedTurns !== evaluationItems.length) {
    failures.push(`all ${evaluationItems.length} turns must be judged by OpenAI without local fallback`);
  }
  ...
  if (new Set(responseIds).size !== evaluationItems.length * 3) {
    failures.push(`production evaluation OpenAI response IDs must be unique across all ${evaluationItems.length * 3} calls`);
  }
  ...
  const expectedTurns = new Set(evaluationItems.map((item) => item.turn));
  if (expected.turns.some((turn) =>
    !expectedTurns.has(turn.turn) ||
    evaluationItems.find((item) => item.turn === turn.turn)?.expectedLevel !== turn.expectedLevel
  ) || new Set(expected.turns.map((turn) => turn.turn)).size !== evaluationItems.length) {
    failures.push(`production evaluation must preserve all ${evaluationItems.length} unique turn numbers and expected levels`);
  }
  return failures;
}
