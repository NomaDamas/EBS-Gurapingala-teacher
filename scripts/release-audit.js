...
const classroomConfigEvidence = readJsonEvidence(classroomConfigEvidenceFile, "CLASSROOM_CONFIG_EVIDENCE_FILE");
if (requireClassroomConfig && !classroomConfigEvidenceFile) {
  failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE is required and must be generated by npm run rehearsal:config against the filming room");
} else if (classroomConfigEvidence) {
  if (classroomConfigEvidence.schemaVersion !== "classroom-config-evidence/v1") {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE schemaVersion must be classroom-config-evidence/v1");
  }
  if (normalizeStatus(classroomConfigEvidence.status) !== "pass") {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE status must be pass");
  }
  if (!classroomConfigEvidence.prHeadSha || classroomConfigEvidence.prHeadSha !== prHeadSha) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE prHeadSha must match PR_HEAD_SHA");
  }
  if (normalizeBaseUrl(classroomConfigEvidence.workerUrl) !== normalizeBaseUrl(workerUrl)) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE workerUrl must match WORKER_URL");
  }
  if (!isFilmingRoom(classroomConfigEvidence.roomId)) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE roomId must be a filming/rehearsal room, not default-classroom or deploy-verify");
  }
  if (!Number.isInteger(classroomConfigEvidence.expectedLevel) || classroomConfigEvidence.expectedLevel < 1 || classroomConfigEvidence.expectedLevel > 4) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE expectedLevel must be 1, 2, 3, or 4");
  }
  if (!String(classroomConfigEvidence.expectedPersona || "").trim()) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE expectedPersona is required");
  }
  if (classroomConfigEvidence.requireOpenAI !== true) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE must record requireOpenAI=true");
  }
  if (classroomConfigEvidence.requireTeacherToken !== true) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE must record requireTeacherToken=true");
  }
  if (classroomConfigEvidence.observedConfig?.persona !== classroomConfigEvidence.expectedPersona ||
    ...
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE observedConfig must match expected Level/persona");
  }
  if (!Array.isArray(classroomConfigEvidence.checks) || classroomConfigEvidence.checks.some((check) => check?.passed !== true)) {
    failures.push("CLASSROOM_CONFIG_EVIDENCE_FILE checks must all pass");
  }
}
...
console.log(`classroomConfigEvidenceFile=${classroomConfigEvidenceFile}`);
