...
const modelEvaluationEvidence = readJsonEvidence(modelEvaluationEvidenceFile, "MODEL_EVALUATION_EVIDENCE_FILE");
let modelEvaluationEvidenceGeneratedAt = null;
if (!modelEvaluationEvidenceFile) {
  failures.push("MODEL_EVALUATION_EVIDENCE_FILE is required and must be generated by the real OpenAI 50-turn evaluation");
} else if (modelEvaluationEvidence) {
  if (modelEvaluationEvidence.schemaVersion !== "model-evaluation-evidence/v1") {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE schemaVersion must be model-evaluation-evidence/v1");
  }
  if (modelEvaluationEvidence.prHeadSha !== prHeadSha) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE prHeadSha must match PR_HEAD_SHA");
  }
  modelEvaluationEvidenceGeneratedAt = parseEvidenceTimestamp(modelEvaluationEvidence.generatedAt);
  if (!modelEvaluationEvidenceGeneratedAt) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE generatedAt must be a valid timestamp");
  }
  validateModelEvaluationEvidence(modelEvaluationEvidence);
...
if (!verifyDeployEvidenceFile) {
  ...
  if (!String(deployEvidence.expectedOpenAIVerifierModel || "").trim()) {
    failures.push("VERIFY_DEPLOY_EVIDENCE_FILE must record expectedOpenAIVerifierModel");
  } else if (deployEvidence.health?.openaiVerifierModel !== deployEvidence.expectedOpenAIVerifierModel) {
    failures.push("VERIFY_DEPLOY_EVIDENCE_FILE health.openaiVerifierModel must match expectedOpenAIVerifierModel");
  }
  if (modelEvaluationEvidence &&
    deployEvidence.expectedOpenAIModel !== modelEvaluationEvidence.expectedGeneratorModel) {
  ...
  if (modelEvaluationEvidence &&
    deployEvidence.expectedOpenAIVerifierModel !== modelEvaluationEvidence.expectedVerifierModel) {
...
if (externalReviewGeneratedAt && modelEvaluationEvidenceGeneratedAt && externalReviewGeneratedAt < modelEvaluationEvidenceGeneratedAt) {
  failures.push("EXTERNAL_REVIEW_FILE generatedAt must be after MODEL_EVALUATION_EVIDENCE_FILE generatedAt");
}
...
console.log(`modelEvaluationEvidenceFile=${modelEvaluationEvidenceFile}`);
...
function validateExternalReviewArtifacts(artifacts) {
  ...
  validateEvidenceArtifact(
    artifacts.modelEvaluation,
    modelEvaluationEvidenceFile,
    "EXTERNAL_REVIEW_FILE evidenceArtifacts.modelEvaluation"
  );
  validateExternalReviewModelAttestation(artifacts.modelEvaluation);
  validateEvidenceArtifact(
...
function validateExternalReviewModelAttestation(artifact) {
  if (!artifact || typeof artifact !== "object") return;
  const attestation = artifact.attestation;
  const currentSha256 = modelEvaluationEvidenceFile
    ? hashEvidenceFile(modelEvaluationEvidenceFile, "MODEL_EVALUATION_EVIDENCE_FILE attestation")
    : "";
  if (attestation?.ok !== true ||
    attestation.schemaVersion !== "model-evaluation-attestation/v1" ||
    attestation.prHeadSha !== prHeadSha ||
    attestation.evidenceSha256 !== currentSha256 ||
    attestation.repository !== TRUSTED_MODEL_EVALUATION_REPOSITORY ||
    attestation.workflowPath !== TRUSTED_MODEL_EVALUATION_WORKFLOW) {
    failures.push("EXTERNAL_REVIEW_FILE evidenceArtifacts.modelEvaluation.attestation must bind the current model evidence to the trusted Deploy workflow and PR_HEAD_SHA");
  }
}
...
function validateModelEvaluationEvidence(evidence) {
  if (normalizeStatus(evidence.status) !== "pass") {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE status must be pass");
  }
  if (evidence.requireOpenAIEvaluation !== true) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE must record requireOpenAIEvaluation=true");
  }
  if (evidence.totalTurnsPerModel !== 50) {
  ...
  const expectedSetSha256 = createHash("sha256")
    ...
    .digest("hex");
  if (evidence.evaluationSetSha256 !== expectedSetSha256) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE evaluationSetSha256 must match the current 50-turn set");
  }
  const startedAt = parseEvidenceTimestamp(evidence.startedAt);
  const completedAt = parseEvidenceTimestamp(evidence.completedAt);
  const generatedAt = parseEvidenceTimestamp(evidence.generatedAt);
  ...
  const generatorModel = String(evidence.expectedGeneratorModel || "").trim();
  const verifierModel = String(evidence.expectedVerifierModel || "").trim();
  const judgeModel = String(evidence.expectedJudgeModel || "").trim();
  if (!generatorModel) failures.push("MODEL_EVALUATION_EVIDENCE_FILE expectedGeneratorModel is required");
  if (!verifierModel) failures.push("MODEL_EVALUATION_EVIDENCE_FILE expectedVerifierModel is required");
  if (!judgeModel) failures.push("MODEL_EVALUATION_EVIDENCE_FILE expectedJudgeModel is required");
  if (evidence.recommendedModel?.model !== generatorModel ||
    evidence.recommendedModel?.eligible !== true) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE recommendedModel must be the eligible expectedGeneratorModel");
  }
  const result = Array.isArray(evidence.models)
    ? evidence.models.find((item) => item?.model === generatorModel)
    : null;
  if (!result) {
  ...
  if (result.metrics?.falsehoodRate !== 1 ||
    ...
    Number(result.metrics?.averageSubtletyScore) < Number(evidence.selectionCriteria?.minimums?.averageSubtletyScore) ||
  ...
  if (!Array.isArray(result.turns) || result.turns.length !== 50) {
    failures.push("MODEL_EVALUATION_EVIDENCE_FILE turns must contain 50 teacher-reviewable results");
    return;
  }
...
function validateClassroomConfigEvidence(classroomConfigEvidence, file, seenRooms, reviewGeneratedAt) {
  ...
  if (!String(classroomConfigEvidence.expectedOpenAIVerifierModel || "").trim()) {
    failures.push(`${label} must record expectedOpenAIVerifierModel`);
  } else if (classroomConfigEvidence.observedHealth?.openaiVerifierModel !== classroomConfigEvidence.expectedOpenAIVerifierModel) {
    failures.push(`${label} observedHealth.openaiVerifierModel must match expectedOpenAIVerifierModel`);
  }
  if (deployEvidence &&
    classroomConfigEvidence.expectedOpenAIModel !== deployEvidence.expectedOpenAIModel) {
    failures.push(`${label} expectedOpenAIModel must match VERIFY_DEPLOY_EVIDENCE_FILE expectedOpenAIModel`);
  }
  if (deployEvidence &&
    classroomConfigEvidence.expectedOpenAIVerifierModel !== deployEvidence.expectedOpenAIVerifierModel) {
    failures.push(`${label} expectedOpenAIVerifierModel must match VERIFY_DEPLOY_EVIDENCE_FILE expectedOpenAIVerifierModel`);
  }
  if (modelEvaluationEvidence &&
    classroomConfigEvidence.expectedOpenAIModel !== modelEvaluationEvidence.expectedGeneratorModel) {
    failures.push(`${label} expectedOpenAIModel must match MODEL_EVALUATION_EVIDENCE_FILE expectedGeneratorModel`);
  }
  if (modelEvaluationEvidence &&
    classroomConfigEvidence.expectedOpenAIVerifierModel !== modelEvaluationEvidence.expectedVerifierModel) {
  ...
  if (classroomConfigEvidence.verifyClassroomChat === true &&
    !hasValidSampleClassroomChat(classroomConfigEvidence.sampleChat, classroomConfigEvidence.expectedLevel, classroomConfigEvidence.expectedPersona, classroomConfigEvidence.expectedOpenAIVerifierModel)) {
