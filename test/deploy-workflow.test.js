...
test("Deploy workflow verifies the real Worker with the same strict production gates as release audit", async () => {
  ...
  const classroomSteps = workflow.slice(
    workflow.indexOf("- name: Configure class 3-5"),
    workflow.indexOf("- name: Upload deploy verification evidence")
  );

  assert.match(evaluationStep, /OPENAI_API_KEY: \$\{\{ secrets\.OPENAI_API_KEY \}\}/);
  assert.match(evaluationStep, /LLM_PROVIDER: openai/);
  assert.match(evaluationStep, /EVAL_MODELS: \$\{\{ vars\.EXPECTED_OPENAI_MODEL \|\| 'gpt-5\.6-luna' \}\}/);
  assert.match(evaluationStep, /EXPECTED_OPENAI_VERIFIER_MODEL:/);
  assert.match(evaluationStep, /OPENAI_REASONING_EFFORT:/);
  ...
  assert.match(deployStep, /--var "OPENAI_REASONING_EFFORT:\$OPENAI_REASONING_EFFORT"/);
  assert.match(deployStep, /--var "OPENAI_VERIFIER_REASONING_EFFORT:\$OPENAI_VERIFIER_REASONING_EFFORT"/);
  ...
  assert.match(verifyStep, /EXPECTED_OPENAI_MODEL: \$\{\{ vars\.EXPECTED_OPENAI_MODEL \|\| 'gpt-5\.6-luna' \}\}/);
  assert.match(verifyStep, /EXPECTED_OPENAI_VERIFIER_MODEL:/);
  assert.match(verifyStep, /EXPECTED_OPENAI_REASONING_EFFORT:/);
  assert.match(verifyStep, /EXPECTED_OPENAI_VERIFIER_REASONING_EFFORT:/);
  ...
  assert.match(classroomSteps, /CLASSROOM_ROOM: 3-5/);
  assert.match(classroomSteps, /CLASSROOM_ROOM: 3-1/);
  ...
  assert.match(classroomSteps, /CLASSROOM_CONFIG_EVIDENCE_FILE: artifacts\/3-5-config\.json/);
  assert.match(classroomSteps, /CLASSROOM_CONFIG_EVIDENCE_FILE: artifacts\/3-1-config\.json/);
  assert.match(classroomSteps, /name: classroom-config-evidence/);
  assert.match(classroomSteps, /path: artifacts\/3-\*-config\.json/);
});
