...
test("mixed mode normalizes selected truth and falsehood levels", () => {
  assert.deepEqual(normalizeMixLevels([0, "5", "2", 2, 7, -1, "x"]), [5, 2]);
  assert.deepEqual(normalizeMixLevels([]), [5, 1, 2, 3, 4]);
});

test("mixed mode deterministically rotates truth and selected Levels", () => {
  const config = { responseMode: "mixed", level: 4, mixLevels: [5, 2, 3] };
  assert.deepEqual(selectTurnMode({ ...config, turnIndex: 0 }), { responseMode: "experiment", level: 5 });
  ...
  assert.deepEqual(selectTurnMode({ ...config, turnIndex: 3 }), { responseMode: "experiment", level: 5 });
});
