...
9. 릴리즈 증거 명령을 한 번에 출력해 운영자가 같은 SHA/room/evidence path를 쓰는지 확인한다.
   ...
   - 각 촬영방에서 실제 `/api/chat` 감사 JSON까지 Level/persona 반영 증거를 남기려면 의도적으로 `CLASSROOM_CHAT_PROOF=true`를 추가한다. 이 플래그는 모든 촬영방 `rehearsal:config` 명령에 `VERIFY_CLASSROOM_CHAT=true`를 붙이며, 방마다 `설정검증` 채팅 1턴을 남긴다.
