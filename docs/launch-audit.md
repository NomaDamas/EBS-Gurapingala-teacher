...
- `npm run release:audit`가 최신 PR head SHA, 외부 승인 JSON, 실제 `verify:deploy` 증거 JSON, 각 촬영방 `rehearsal:config` 증거 JSON, `REQUIRE_OPENAI=true`, `REQUIRE_TEACHER_TOKEN=true`, `REQUIRE_CLASSROOM_CONFIG=true` 증거로 통과해야 한다.
