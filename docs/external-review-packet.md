...
- production 50턴 eval이 rules/local-fallback을 사용했거나 response ID가 누락·중복됨.
- production 모델 평가 JSON이 로컬 생성물이거나 GitHub attestation이 현재 PR head의 trusted `Deploy` workflow에 바인딩되지 않음.
...
production 모델 평가 증거는 로컬 `npm run eval` 결과가 아니라 성공한 `Deploy` workflow artifact를 사용한다. 아래 명령에는 `gh attestation` 명령을 지원하는 최신 GitHub CLI가 필요하다.
...
- classroom chat proof mode if required: every room has `verifyClassroomChat=true` and valid `sampleChat` proving `/api/chat` used expected Level/persona
...
리뷰어가 `APPROVE`를 주고 `verify:ci`, attested OpenAI strict `eval`, `eval:set`, 실제 Worker `verify:deploy`, 모든 촬영방 `rehearsal:config`가 같은 PR head에서 pass인 뒤에는 텍스트 판정만 보관하지 말고 `release:audit`가 읽을 JSON 증거를 생성한다. blocking finding이 있거나 `CI_STATUS=success`, `VERIFY_DEPLOY_STATUS=pass`, `CLASSROOM_CONFIG_STATUS=pass`가 아니면 `APPROVE` 증거 생성은 실패한다. `review:evidence`는 CI, 정적 평가 세트, 실제 모델 평가, 배포, 촬영방 증거 파일의 schema, `status=pass`, `prHeadSha`, 촬영방 `roomId`를 확인한 뒤 승인 JSON을 쓴다. `evaluation-set-evidence/v1`은 `totalTurns=50`, teacher-only audit 포함, public projection 정답·거짓 근거 미노출, Level별 preflight 통과를 기록해야 한다. `model-evaluation-evidence/v1`은 실제 OpenAI generator/verifier/judge가 50/50을 통과했고 fallback·blocked turn이 0이며 150개 OpenAI response ID가 모두 존재하고 고유함을 증명해야 한다. 또한 현재 파일 SHA-256이 trusted `.github/workflows/deploy.yml`, `workflow_dispatch`, 현재 PR head SHA의 GitHub attestation에 바인딩되어야 한다. 배포 증거는 `requireOpenAI=true`, `requireTeacherToken=true`, `requireCloudflareEdge=true`, Cloudflare response header evidence, sanitized `/api/health` snapshot, 실제 chat의 `verifierApproved=true`, 모델 평가와 같은 generator/verifier model을 기록해야 한다. `EXPECTED_CLASSROOM_ROOMS`는 촬영 계획 기준 room 목록이며, 승인 JSON 생성 단계에서도 `CLASSROOM_CONFIG_EVIDENCE_FILES`의 `roomId` 집합과 정확히 일치해야 한다. `CLASSROOM_CHAT_PROOF=true`로 릴리즈 명령을 만든 경우 승인 증거 생성에도 `REQUIRE_CLASSROOM_CHAT_PROOF=true`를 넣어 모든 촬영방 evidence의 `sampleChat`을 필수로 만든다. CI 증거는 `checkRun.completedAt`이 `checkRun.startedAt` 이후이고 `generatedAt`이 `checkRun.completedAt` 이후여야 한다. 최종 감사는 외부 리뷰 증거의 `generatedAt`이 CI, 정적 평가 세트, 실제 모델 평가, 배포, 촬영방 증거 생성 시각보다 늦은지도 확인한다.
...
생성되는 JSON은 `external-review-evidence/v1`이며 `decision`, `reviewer`, `source`, `prHeadSha`, `evidenceArtifacts`, `evidenceChecked`, `blockingFindings`, `nonBlockingRisks`를 포함한다. `evidenceArtifacts`에는 리뷰어가 승인 전에 확인한 실제 CI, 정적 50턴 평가 세트, OpenAI 50턴 모델 평가, 배포, 촬영방 증거 파일의 SHA-256 해시가 기록되어야 한다.
